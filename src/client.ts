...
import { ProtocolManager, ProtocolType, ProtocolNegotiationResult } from './protocols/manager';
import { 
  CrabCacheConfig, 
  CrabCacheValue, 
  CrabCacheStats,
...
export class CrabCacheClient extends EventEmitter {
  ...
  private pool!: ConnectionPool; // Use definite assignment assertion
  private clusterNodes: Map<string, ClusterNode> = new Map();
  private nodeConnections: Map<string, ConnectionPool> = new Map();
  private currentNodeIndex: number = 0;
  private healthCheckInterval?: NodeJS.Timeout;
  private protocolManager: ProtocolManager;
  private protocolNegotiated: boolean = false;
  ...
  private initializeCluster(): void {
    ...
    this.config.nodes.forEach((node, index) => {
      ...
      const nodePool = new ConnectionPool(nodeConfig);
  ...
  private async performHealthChecks(): Promise<void> {
    const healthCheckPromises = Array.from(this.clusterNodes.entries()).map(
      async ([nodeId, node]) => {
        try {
          ...
          const encodedCommand = ProtocolEncoder.encodeTextCommand('PING', []);
  ...
  private selectNode(): ClusterNode | null {
  ...
  private selectResourceBased(nodes: ClusterNode[]): ClusterNode {
  ...
  private selectAdaptive(nodes: ClusterNode[]): ClusterNode {
  ...
  async connect(): Promise<void> {
    try {
      if (this.config.enableCluster) {
        ...
        const connectPromises = Array.from(this.nodeConnections.entries()).map(
          async ([nodeId, pool]) => {
            try {
              await pool.warmUp(2);
   ...
   */
  private async negotiateProtocol(): Promise<ProtocolNegotiationResult> {
    ...
    try {
      ...
      const sendCommand = async (data: Buffer): Promise<Buffer> => {
        return await negotiationConnection.sendCommand(data);
      };

      const result = await this.protocolManager.negotiateProtocol(
        sendCommand,
   ...
   */
  private async negotiateProtocolForNode(nodeId: string, pool: ConnectionPool): Promise<void> {
    try {
      const connection = await pool.acquire();
      ...
      const nodeProtocolManager = new ProtocolManager();
      const result = await nodeProtocolManager.negotiateProtocol(
        sendCommand,
        this.config.preferredProtocol
      );
  ...
  private async getClusterStats(): Promise<ClusterStats> {
    const nodeStats: CrabCacheStats[] = [];
    const activeNodes = Array.from(this.clusterNodes.values()).filter(n => n.active);
    ...
    for (const node of activeNodes) {
      try {
        ...
        const connection = await pool.acquire();
        const encodedCommand = ProtocolEncoder.encodeTextCommand('STATS', []);
        const responseBuffer = await connection.sendCommand(encodedCommand);
        pool.release(connection);
        
        const response = ProtocolDecoder.decodeTextResponse(responseBuffer);
        if (response.type === 'stats') {
  ...
  pipeline(): CrabCachePipeline {
    ...
    return new CrabCachePipeline(
      pool as any, // Hack para compatibilidade
   ...
   */
  private fallbackEncodeCommand(command: string, args: (string | Buffer)[]): Buffer {
   ...
   */
  private fallbackDecodeResponse(responseBuffer: Buffer): any {
    if (this.config.useBinaryProtocol) {
      return ProtocolDecoder.decodeBinaryResponse(responseBuffer);
    } else {
      return ProtocolDecoder.decodeTextResponse(responseBuffer);
    }
  }
   ...
   * Update protocol usage metrics
   */
  private updateProtocolUsageMetrics(): void {
  ...
  getProtocolInfo(): {
    ...
    return {
      ...
      capabilities: Object.keys(protocolInfo.capabilities).filter(
   ...
   */
  async resetProtocol(): Promise<ProtocolNegotiationResult> {

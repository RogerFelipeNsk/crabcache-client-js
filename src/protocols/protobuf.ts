...
export const PROTOBUF_MAGIC = Buffer.from([0x43, 0x52, 0x41, 0x42]);
export const PROTOBUF_VERSION = 1;

// Protobuf Flags
export const PROTOBUF_FLAGS = {
  ZERO_COPY: 0x01,
  COMPRESSION: 0x02,
  RESERVED: 0x04,
  RESERVED2: 0x08
...
export class ProtobufProtocol {
  ...
  static createNegotiationPacket(): Buffer {
    ...
    PROTOBUF_MAGIC.copy(packet, 0);
    packet[4] = PROTOBUF_VERSION;
    packet[5] = PROTOBUF_FLAGS.ZERO_COPY; // Enable zero-copy by default
    return packet;
  }
  ...
  static parseNegotiationResponse(response: Buffer): ProtobufNegotiationResult {
    try {
      ...
      if (response.length >= 4 && response.subarray(0, 4).equals(PROTOBUF_MAGIC)) {
  ...
  encodeCommand(command: string, args: (string | Buffer)[] = []): Buffer {
    ...
    buffers.push(PROTOBUF_MAGIC);
    buffers.push(Buffer.from([PROTOBUF_VERSION, PROTOBUF_FLAGS.ZERO_COPY]));
   ...
   */
  private createTextCommand(command: string, args: (string | Buffer)[]): Buffer {
  ...
  decodeResponse(data: Buffer): any {
    ...
    if (data.length >= 4 && data.subarray(0, 4).equals(PROTOBUF_MAGIC)) {
   ...
   */
  private parseTextResponse(data: Buffer): any {
  ...
  getStats(): {
    ...
    return {
      ...
      protocolVersion: PROTOBUF_VERSION
    };
  }
